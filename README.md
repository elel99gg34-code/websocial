...
**Node·터미널·설치가 전부 필요 없다.** [`standalone/websocial.html`](standalone/websocial.html) 를 내려받아
**더블클릭**하면 브라우저에서 열린다. 설정에서 키를 넣고 아이디어 한 줄만 적으면
기획 → 설정집 → 회차표 → 집필 → 검수 → 퇴고 → 캐논 갱신까지 화면에서 돈다.

설계 원칙(브라우저에서 막히는 일을 줄이기 위한 것):

- **스트리밍을 쓰지 않는다.** 평범한 POST 하나와 JSON 응답 하나뿐이라 실패 지점이 적다.
- **커스텀 헤더를 최소로 쓴다.** Gemini 는 키를 주소에 실어 보내 사전 요청(preflight)에서 막힐 여지를 없앴다.
- **실패를 감추지 않는다.** 요청 주소·HTTP 상태·서버 응답 원문을 화면에 그대로 보여주고,
  실패하면 [진단]이 자동으로 이어 돌아 원인을 인터넷/CORS/키/모델 중 하나로 좁혀 준다.

제공자는 Gemini · Anthropic · Groq · xAI Grok · OpenRouter · 직접 입력(Ollama 등) 여섯 곳이며,
각각 키·Base URL·모델을 따로 저장한다. [모델] 버튼이 그 키로 쓸 수 있는 모델을 받아와 채워 준다.

데이터는 그 브라우저의 localStorage 에만 저장되고, 키는 파일 안에 들어가지 않는다.
